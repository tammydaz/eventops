...
// 5 pieces per guest is FoodWerx standard for passed appetizers
const PASSED_APP_PIECES_PER_GUEST = 5;
...
function calculatePassedAppSpec(params: SpecParams): string {
  const totalPieces = params.guestCount * PASSED_APP_PIECES_PER_GUEST;
...
function isSauce(itemName: string): boolean {
  const lowerName = itemName.toLowerCase();
  return ['sauce', 'dressing', 'aioli', 'vinaigrette', 'reduction'].some(k => lowerName.includes(k));
}
