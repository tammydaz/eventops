...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!apiKey || !baseId || !tableId || !jwtSecret) {
    console.error("Missing env: AIRTABLE_API_KEY, AIRTABLE_BASE_ID, AIRTABLE_USERS_TABLE, AUTH_JWT_SECRET");
  ...
  try {
    ...
    if (record.fields.PasswordHash) {
      return res.status(400).json({ error: "Password already set. Use the login form." });
    }
